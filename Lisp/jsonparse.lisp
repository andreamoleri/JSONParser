...
(defun opening-quote-handling (JSONString char-obj)
  ...
  (append '(jsonobj) (jsonmembers JSONString)))
...
;;; The function is auxiliary to jsonmembers and the two are used 
...
(defun create-list (input)
  (let ((nextcharacter (read-char (space-cleanup input) T)))
    (cond ((is-comma nextcharacter) (jsonmembers (space-cleanup input)))
...
(defun write-value (input output)
  (cond
...
;;; Auxiliary to write-value
(defun output-structure (input output)
  (cond
   ((and (typep input 'list) (equal (car input) 'jsonarray))
    (output-arr (mapcar 
      (lambda (x) (write-value x NIL)) (cdr input)) output))
   ((and (typep input 'list) (equal (car input) 'jsonobj))
    (output-obj (mapcar 
      (lambda (y) (output-kvp (car y) (write-value (second y) NIL))) 
        (cdr input)) output))
...
(defun jsonmembers (JSONString)
  (let ((nextcharacter (read-char (space-cleanup JSONString) T)))
    (if (is-opening-quote nextcharacter)
        (cons (jsonpair JSONString) (create-list JSONString))
        (error "The jsonmembers function is unable to parse~@ 
...
(defun jsonpair (input)
  (let ((key (jsonstring input)))
    (when (is-colon (read-char (space-cleanup input) T))
      (list key (jsonvalue (space-cleanup input))))))
     ...
     ((lambda (nextcharacter)
      (cond ((is-closing-bracket nextcharacter) (quote (jsonarray)))
            (T
             ...
             (cons 'jsonarray (jsonelements input)))))
             (read-char (space-cleanup input) T)))))

;;; Auxiliary to jsonarray
(defun jsonelements (input)
  (append (list (jsonvalue input))
    ((lambda (nextcharacter)
      (if (is-closing-bracket nextcharacter) NIL
      (if (is-comma nextcharacter) (jsonelements (space-cleanup input))
    (error "Invalid character found, unable to create JSON elements list"))))  
...
;;; is used when find-jsonvalue is called within this function
(defun jsonaccess (JSONObject &rest fields)
  (cond ((not (listp JSONObject)) (access-error))
        ...
        (T (find-jsonvalue JSONObject fields))))
